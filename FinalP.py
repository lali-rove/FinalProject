...
# function that creates a banner(I created) and title to include in every page
def Intro():
    st.image('intro.jpg')
    st.title("Welcome to the Skyscraper's Database!")


# function that creates a list of the materials used for each building
def materials():
...
# function that creates a list of the floors and height each building has
...
# function that creates a list of the names of the skyscrapers
def skyscraperNames():
...
# function that creates the scatter plot figures for the Scatter Plot Page
def ScatterPlot(dataframe, FRange):
    if FRange == 'Low':
        color = st.color_picker('Pick A Color', '#861F8A')
        df3 = df2[(df2.Floors >= 50) & (df2.Floors <= 80)]
        floor_list, height_list = floors(df3)
        fig, ax = plt.subplots()
        ax.scatter(floor_list, height_list, color=color)
        ax.set_xlabel('No. of Floors')
        ax.set_ylabel('Height in Feet')
        ax.set_title('Skyscraper Scatter Plot')
        st.pyplot(fig)
    elif FRange == 'Mid':
        color = st.color_picker('Pick A Color', '#CB6D51')
        ...
        color = st.color_picker('Pick A Color', '#DBA0A9')
...
# SideBar Menu
st.sidebar.image("SkyData.jpg")  # Used the image I created as a title, looks good
...
if pages == "HomePage":
    Intro()
    st.header("To select different pages click on the SideBar.")
